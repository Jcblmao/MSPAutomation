...
param(
    ...
    # Upload the installers to an Azure Blob Storage container.
    [Switch]$Upload,
...
$Locations = ForEach ($Organisation in $Organisations) {
    ...
    ForEach ($Location in $Locations) {
        ...
        $InstallerURI = New-NinjaOneInstaller -organisationId $Organisation.id -locationId $Location.id -installerType 'WINDOWS_MSI' | Select-Object -ExpandProperty 'url'
        $Installer = @{
...
$Organisations = Get-NinjaOneOrganisations
$InstallerDetails = foreach ($Organisation in $Organisations) {
    $Locations = Get-NinjaOneLocations -organisationID $Organisation.id
        foreach ($Location in $Locations) {
        Write-Host ('O: {0} | L: {1}' -f $Organisation.Name, $Location.Name)
        $Installer = Get-NinjaOneInstaller -organisationID $Organisation.id -locationID $Location.id -installerType WINDOWS_MSI
        [PSCustomObject]@{
            Organisation = $Organisation.Name
            Location = $Location.Name
            InstallerURI = $Installer
        }
    }
}
$InstallerDetails | Export-Csv -Path $CSVPath